...
const getMarkerIcon = (type, status) => {
  ...
  const size = type === 'NRO' ? 18 : (type.startsWith('SR') ? 14 : 10);
...
function OfflineControl() {
  ...
  return (
    <div className="map-tools-stack">
      ...
      <div style={{ height: '4px' }}></div>
      {!isDownloading ? (
        <button className="tool-btn" onClick={() => {}} title="Offline Ready"><Download size={20} /></button>
      ) : (
          ...
          >
            <Popup>
              <div style={{ textAlign: 'center', minWidth: '120px' }}>
                <strong style={{ display: 'block', fontSize: '14px' }}>{eq.node_id}</strong>
                <span style={{ fontSize: '11px', color: '#666', textTransform: 'uppercase' }}>{eq.type.replace('_', ' ')}</span>
                <hr style={{ margin: '8px 0', opacity: 0.1 }} />
                <div style={{ fontSize: '10px', textAlign: 'left' }}>
                  <b>Statut:</b> {eq.status.toUpperCase()}<br/>
                  <b>Installation:</b> {eq.metadata?.installation_date || 'N/A'}
                </div>
              </div>
            </Popup>
          </Marker>
